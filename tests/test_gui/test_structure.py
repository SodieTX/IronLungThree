...
def test_gui_app_structure():
    ...
    required_methods = {
        "__init__", "run", "_create_window", "_create_tabs",
        "_create_status_bar", "_bind_shortcuts", "close",
        "_on_tab_changed", "_update_status_bar"
    }
...
def test_import_tab_structure():
    ...
    required_methods = {
        "__init__", "refresh", "on_activate", "select_file",
        "preview_import", "execute_import", "_create_ui"
    }
...
def test_pipeline_tab_structure():
    ...
    method_names = {n.name for n in tab_class.body if isinstance(n, ast.FunctionDef)}
    required_methods = {
        "__init__", "refresh", "on_activate", "apply_filters",
        "export_view", "bulk_move", "bulk_park", "_create_ui"
    }
...
def test_theme_structure():
    ...
    var_names = {n.targets[0].id for n in tree.body 
