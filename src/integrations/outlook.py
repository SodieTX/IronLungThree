...
class OutlookClient(IntegrationBase):
    ...
    def get_sent_emails(
        ...
        """Get sent emails via Graph API.

        Args:
            since: Only messages sent after this time
            limit: Maximum messages to return

        Returns:
            List of sent email messages, newest first

        Raises:
            OutlookError: If sent items read fails
        """
        self._ensure_authenticated()
        ...
        params: dict[str, str] = {
            ...
            "$select": ("id,from,toRecipients,subject,body,bodyPreview,sentDateTime"),
        }

        if since:
            iso_since = since.strftime("%Y-%m-%dT%H:%M:%SZ")
            params["$filter"] = f"sentDateTime ge {iso_since}"

        try:
            response = self._graph_request(
                "GET",
                f"/users/{self._user_email}/mailFolders/sentitems/messages",
                params=params,
            )

            if response.status_code != 200:
                raise OutlookError(
                    f"Sent items read failed ({response.status_code}): {response.text}"
                )
            ...
            for msg in data.get("value", []):
                ...
                if msg.get("sentDateTime"):
                    sent_at = datetime.fromisoformat(msg["sentDateTime"].replace("Z", "+00:00"))
            ...
            logger.info(
                f"Retrieved {len(messages)} sent messages",
                extra={"context": {"count": len(messages), "since": str(since)}},
            )
            return messages
            ...
            raise OutlookError(f"Failed to read sent items: {e}") from e
