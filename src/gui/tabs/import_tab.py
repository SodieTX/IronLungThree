...
class ImportTab(TabBase):
    ...
    def __init__(self, parent: tk.Widget, db: object):
        ...
        self._mapping_widgets: Dict[str, tk.StringVar] = {}
        self._create_ui()
    ...
    def _load_file(self) -> None:
        ...
        try:
            ...
            parse_result = importer.parse_file(Path(self._selected_file))
            self._parse_result = parse_result

            # Update preset dropdown
            if parse_result.detected_preset:
                self._preset_var.set(parse_result.detected_preset)
            ...
            logger.info(
                f"File parsed: {parse_result.total_rows} rows, "
                f"{len(parse_result.headers)} columns"
            )
    ...
    def _show_mapping_ui(self) -> None:
        ...
        for i, (field_key, field_label) in enumerate(required_fields):
            ...
            for header in self._parse_result.headers:
                ...
                if (
                    field_lower in header_lower.split()
                    or field_key_lower in header_lower.replace("_", " ").split()
                ):
                    var.set(header)
                    break
    ...
    def _show_preview_dialog(self, preview: Any) -> None:
        ...
        summary_text = (
            ...
            f"Will be merged: {len(preview.merge_records)}\n"
            f"Needs review: {len(preview.needs_review)}\n"
            f"Blocked (DNC): {len(preview.blocked_dnc)}\n"
            f"Incomplete: {len(preview.incomplete)}\n"
        )
    ...
    def execute_import(self) -> None:
        ...
        try:
            ...
            # Confirm import
            total_rows = self._parse_result.total_rows
            filename = Path(self._selected_file).name
            if not messagebox.askyesno(
                "Confirm Import",
                f"Import {total_rows} records from {filename}?",
            ):
                return
            ...
            logger.info(
                f"Import completed: {result.imported_count} created, "
